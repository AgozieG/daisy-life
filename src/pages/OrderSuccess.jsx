...
export default function OrderSuccess() {
  ...
  useEffect(() => {
    ...
    confetti({ particleCount: 120, spread: 90, origin: { y: 0.4 }, colors: ['#E34B36', '#B9322B', '#4D9A70', '#F47A32'] });
  ...
  return (
    <div className="pt-28 pb-16 px-4 sm:px-6 max-w-lg mx-auto min-h-screen flex flex-col items-center text-center">
      ...
      <h1 className="font-display text-3xl sm:text-4xl font-bold text-charcoal mt-5 mb-2">Order Placed!</h1>
      <p className="text-charcoal/60 font-body mb-6">
      ...
      <button onClick={copyId} className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-full px-4 py-2 text-charcoal/80 text-sm font-body mb-8">
      ...
      <div className="w-full bg-charcoal-light rounded-2xl p-5 mb-8 text-left">
        <h3 className="font-accent font-bold text-charcoal mb-4">Order Status</h3>
        <div className="space-y-4">
          {STAGES.map((s, i) => (
            <div key={s} className="flex items-center gap-3">
              ...
              <span className={`text-sm font-body ${i <= stageIndex ? 'text-charcoal' : 'text-charcoal/40'}`}>{s}</span>
            </div>
          ))}
        </div>
        <p className="text-charcoal/50 text-xs font-body mt-4">
      ...
      <div className="w-full flex flex-col gap-3">
        ...
        <button onClick={() => navigate('/')} className="text-charcoal/60 text-sm font-body underline underline-offset-2">
          Back to Home
        </button>
      </div>
    </div>
